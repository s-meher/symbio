...
export default function DashboardBorrower() {
  ...
  const paymentSchedule = useMemo(() => {
    ...
    if (totalOwed <= 0 && nextAmount <= 0) return [];
    const baselineBalance = totalOwed > 0 ? totalOwed : nextAmount * 12;
    const monthlyInstallment = baselineBalance / 12;
    let remainingBalance = baselineBalance;
    return Array.from({ length: 12 }, (_, index) => {
      const taper = 1 - Math.min(index * 0.04, 0.3);
      const projectedPayment = Math.max(monthlyInstallment * taper, monthlyInstallment * 0.55);
      const payment = Math.round(Math.min(projectedPayment, remainingBalance));
      remainingBalance = Math.max(remainingBalance - payment, 0);
      return {
        label: `Month ${index + 1}`,
        payment,
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="default" onClick={() => navigate('/dashboard/lender')}>
            Lender dashboard
          </Button>
        </div>
      </div>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <Card className="rounded-3xl border border-border/60 bg-white/90 shadow-sm">
                <CardHeader>
                  ...
                  <CardDescription>Projected monthly installments and balance decline</CardDescription>
                </CardHeader>
                <CardContent className="h-[300px]">
                  {paymentSchedule.length ? (
                    <ChartContainer config={paymentChartConfig} className="h-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={paymentSchedule} margin={{ left: 16, right: 8, top: 8, bottom: 0 }}>
                          <defs>
                      ...
                      <div className="mt-4 flex flex-wrap gap-4 text-sm text-muted-foreground">
                        <div className="flex items-center gap-2">
                          <span className="h-2 w-2 rounded-full" style={{ backgroundColor: 'var(--color-payment)' }} />
                          <span>{paymentChartConfig.payment.label}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="h-2 w-2 rounded-full" style={{ backgroundColor: 'var(--color-balance)' }} />
                          <span>{paymentChartConfig.balance.label}</span>
                        </div>
                      </div>
                    </ChartContainer>
                  ) : (
                ...
                <CardContent className="h-[300px]">
                  {savingsGrowth.length ? (
                    <ChartContainer config={savingsChartConfig} className="h-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={savingsGrowth} margin={{ left: 16, right: 8, top: 8, bottom: 0 }}>
                          <defs>
